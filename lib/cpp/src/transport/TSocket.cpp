...
uint32_t TSocket::read(uint8_t* buf, uint32_t len) {
 ...
 try_again:
  ...
  if (got < 0) {
    ...
    GlobalOutput.perror("TSocket::read() recv() " + getSocketInfo(), errno);
...
void TSocket::setLinger(bool on, int linger) {
  ...
  if (ret == -1) {
    GlobalOutput.perror("TSocket::setLinger() setsockopt() " + getSocketInfo(), errno);
  }
}
...
void TSocket::setNoDelay(bool noDelay) {
  ...
  if (ret == -1) {
    GlobalOutput.perror("TSocket::setNoDelay() setsockopt() " + getSocketInfo(), errno);
  }
}
...
void TSocket::setRecvTimeout(int ms) {
  ...
  if (ret == -1) {
    GlobalOutput.perror("TSocket::setRecvTimeout() setsockopt() " + getSocketInfo(), errno);
  }
}
...
void TSocket::setSendTimeout(int ms) {
  ...
  if (ret == -1) {
    GlobalOutput.perror("TSocket::setSendTimeout() setsockopt() " + getSocketInfo(), errno);
  }
}
